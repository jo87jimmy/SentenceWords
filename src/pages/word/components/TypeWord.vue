...
const showFullWord = ref<boolean>(false)
...
const displayWord = computed(() => {
  ...
  const currentWrong = wrong.value;
  return props.word.word.slice(currentInput.length + currentWrong.length);
})
      ...
      if ([WordPracticeType.FollowWrite, WordPracticeType.Spell].includes(settingStore.wordPracticeType)) {
        if (settingStore.autoNextWord) {
          if (settingStore.repeatCount == 100) {
            if ((settingStore.repeatCustomCount || 0) <= wordRepeatCount + 1) {
              jumpTimer = setTimeout(() => emit('complete'), settingStore.waitTimeForChangeWord) as any
            } else {
              repeat()
            }
          } else {
...
function checkCursorPosition() {
  _nextTick(() => {
    ...
    if (!typingWordRef.value || !cursorEl) return;
