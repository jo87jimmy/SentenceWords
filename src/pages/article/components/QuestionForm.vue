...
// 定义子组件实例接口
interface QuestionItemComp {
  submit: () => void
  getResult: () => QuestionResult
}

const questionRefs = useTemplateRef<QuestionItemComp[]>('questionRefs1')
...
const onAnswered = (res: { index: number; selected: string[]; isCorrect: boolean }) => {
  console.log('Answered:', res)
  // 可收集中间过程（非必须）
}

const submitAll = () => {
  console.log(questionRefs.value)
  if (!questionRefs.value) return
