...
import {_getStudyProgress, cloneDeep, last, _nextTick, _parseLRC} from "@/utils";
...
import {getDefaultArticle} from "@/types/func";
...
const networkTranslateEngine = ref(TranslateEngine.Baidu)
...
const editArticle = ref<Article>(cloneDeep(props.article))
...
watch(() => editArticle.value.text, (s) => {
  if (!s.trim()) {
...
let preSentence = ref<Sentence | null>(null)
...
function handleShowEditAudioDialog(val: Sentence, i: number, j: number) {
  ...
  if (j == 0) {
    if (i != 0) {
      preSentence.value = last(editArticle.value.sections[i - 1]) || null
    }
  } else {
...
function setStartTime(val: Sentence, i: number, j: number) {
  ...
  if (j == 0) {
    if (i != 0) {
      preSentence = last(editArticle.value.sections[i - 1]) || null
    }
  } else {
      ...
      <template v-if="editArticle?.sections?.length">
        <div class="flex-1 overflow-auto flex flex-col">
          ...
          <div class="article-translate">
            <div class="section  rounded-md " v-for="(item,indexI) in editArticle.sections" :key="indexI">
              <div class="section-title text-lg font-bold">第{{ indexI + 1 }}段</div>
              <div class="sentence" v-for="(sentence,indexJ) in item" :key="indexJ">
                <div class="flex-[7]">
                  <EditAbleText
                    :disabled="![100,0].includes(progress)"
                    :value="sentence.text"
