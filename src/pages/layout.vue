  ...
  <div class="flex w-full h-full bg-surface-0 dark:bg-surface-900 overflow-hidden">
    ...
    <div
      ...
      <div class="flex justify-evenly w-full pb-2">
        ...
        <BaseIcon v-if="settingStore.sideExpand"
          :title="`切換主題(${settingStore.shortcutKeyMap[ShortcutKey.ToggleTheme]})`" @click="toggleTheme"
          class="hover:bg-surface-200 dark:hover:bg-surface-700 p-1.5 rounded-md cursor-pointer transition-colors text-surface-700 dark:text-surface-200">
    ...
    <!-- 行動端頂部選單欄 -->
    <div
      class="md:hidden fixed top-0 left-0 right-0 bg-surface-50 dark:bg-surface-800 border-b border-surface-200 dark:border-surface-700 shadow-sm z-50 transition-transform duration-300 ease-in-out"
      :class="{ '-translate-y-[calc(100%-1.5rem)]': settingStore.mobileNavCollapsed }">
      <div class="flex justify-around py-2 transition-opacity duration-200"
        :class="{ 'opacity-0 pointer-events-none': settingStore.mobileNavCollapsed }">
        <div
          ...
          @click="router.push('/')" :class="{ 'text-primary-500 dark:text-primary-400': $route.path === '/' }">
