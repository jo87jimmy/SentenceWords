...
// 设置密码
export function setPassword(data: { password: string }) {
  return http('user/setPassword', data, null, 'post')
}

// 修改邮箱
export function changeEmailApi(data: { email: string }) {
  return http('user/changeEmail', data, null, 'post')
}

// 修改手机号
export function changePhoneApi(data: { phone: string }) {
  return http('user/changePhone', data, null, 'post')
}

// 修改用户信息
export function updateUserInfoApi(data: { nickname: string }) {
