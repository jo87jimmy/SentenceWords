...
let currentList = computed(() => {
  ...
  return list.value.slice(
    ...
    (pageNo.value - 1) * pageSize.value + pageSize.value
  );
});
...
let selectAll = computed({
  get: () =>
    ...
    selectIds.value.length === currentList.value.length,
  set: () => toggleSelectAll(),
});
...
<template>
  <div class="flex flex-col gap-3">
    ...
    <div
      v-if="loading"
      class="h-full w-full flex justify-center items-center text-4xl"
    >
    ...
    >
      <div class="w-full p-4 pt-0">
        ...
        <div>
          翻譯：一行一個翻譯，前面詞性，後面內容（如n.取消）；多個翻譯請換行
        </div>
        <div>
          例句：一行原文，一行譯文；多個請換<span class="text-red-500">兩</span
          >行
        </div>
        <div>
          短語：一行原文，一行譯文；多個請換<span class="text-red-500">兩</span
          >行
        </div>
        <div>
          同義詞、同根詞、詞源：請前往官方字典，然後編輯其中某個單詞，參考其格式
        </div>
        <div class="mt-6">
          模板下載地址：<a
            :href="`https://${Host}/libs/单词导入模板.xlsx`"
