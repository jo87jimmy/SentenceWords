...
const progress = computed(() => {
  ...
  const lastLearnIndex = props.item?.lastLearnIndex ?? 0
  const length = props.item?.length ?? 1
  return Number(((lastLearnIndex / length) * 100).toFixed())
})

const studyProgress = computed(() => {
  if (!props.showProgress) return
  if (props.item?.complete) return (props.item?.length || 0) + '/'
