import { offset } from "@floating-ui/dom"; // 引入 floating-ui 的 offset 中間件，用於處理浮動元素的位移

export const GITHUB = 'https://github.com/zyronon/TypeWords' // GitHub 專案地址常數
export const Host = 'typewords.cc' // 應用程式的主機域名
export const EMAIL = 'zyronon@163.com' // 聯絡電子郵件地址
export const Origin = `https://${Host}` // 應用程式的完整來源 URL (協議 + 主機)
export const APP_NAME = 'Type Words' // 應用程式名稱常數

const common = { // 定義通用的配置物件
    word_dict_list_version: 1 // 單字字典列表的版本號
}
const map = { // 定義不同環境的配置映射
    DEV: { // 開發環境配置
        API: 'http://localhost/', // 開發環境的 API 基礎地址
    }
}

export const ENV = Object.assign(map['DEV'], common) // 導出環境變數，將開發環境配置與通用配置合併

export let AppEnv = { // 導出應用程式運行時環境狀態
...
// console.log('AppEnv.CAN_REQUEST',AppEnv.CAN_REQUEST) // 調試日誌：輸出是否可發送請求的狀態

export const RESOURCE_PATH = ENV.API + 'static' // 靜態資源的基礎路徑

export const DICT_LIST = { // 字典列表的 API 路徑配置
    WORD: { // 單字相關字典列表
...
export const APP_VERSION = { // 應用程式版本資訊物件
    key: 'type-words-app-version', // 存儲版本的鍵名
    version: 2 // 當前版本號
}
export const SAVE_DICT_KEY = { // 儲存字典資訊的鍵名配置
    key: 'typing-word-dict', // 儲存字典的鍵名
    version: 4 // 儲存結構的版本號
}
export const SAVE_SETTING_KEY = { // 儲存設定資訊的鍵名配置
    key: 'typing-word-setting', // 儲存設定的鍵名
    version: 17 // 儲存結構的版本號
}
export const EXPORT_DATA_KEY = { // 匯出資料的鍵名配置
    key: 'typing-word-export', // 匯出資料的鍵名
    version: 4 // 資料結構版本號
}
export const LOCAL_FILE_KEY = 'typing-word-files' // 本地檔案存儲的鍵名

export const PracticeSaveWordKey = { // 練習中儲存單字的鍵名配置
    key: 'PracticeSaveWord', // 鍵名
    version: 1 // 版本號
}
export const PracticeSaveArticleKey = { // 練習中儲存文章的鍵名配置
    key: 'PracticeSaveArticle', // 鍵名
    version: 1 // 版本號
}

export const TourConfig = { // 導覽教學的配置物件
    ...
    defaultStepOptions: { // 預設步驟選項
        ...
        floatingUIOptions: { // Floating UI 的配置選項
...
export const LIB_JS_URL = { // 第三方 JS 庫的 URL 配置
